...
/**
 * Listens for `push-token` events emitted by the native driver shell
 * (see ma-removals-driver-android/src/screens/DriverApp.tsx) and posts
 * them to `/api/driver/push/register`. The endpoint is authenticated
...
export function NativePushRegister() {
  useEffect(() => {
    ...
    const unsubscribe = bridge.onEvent(async (evt) => {
      ...
      try {
        const res = await fetch("/api/driver/push/register", {
          method: "POST",
